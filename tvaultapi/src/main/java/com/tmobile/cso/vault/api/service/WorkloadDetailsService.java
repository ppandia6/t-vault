...
public class WorkloadDetailsService {
	...
	public ResponseEntity<String> getWorkloadDetails(String token, UserDetails userDetails) {
		...
		workloadAppDetailsList.add(new WorkloadAppDetails(WorkloadAppDetails.APP_NAME_OTHER,
				WorkloadAppDetails.APP_TAG_OTHER, WorkloadAppDetails.APP_ID_OTHER));

		// get first response
		String response = null;
		try {
			response = adminservice.getTMOAppMetadataDetails();
	...
	private void populateWorkloadAppDetails(List<WorkloadAppDetails> workloadAppDetailsList, JsonElement jsonElement) {
		if (jsonElement.getAsJsonObject() != null) {
			JsonObject spec = jsonElement.getAsJsonObject();
			WorkloadAppDetails workloadAppDetails = new WorkloadAppDetails();
			if (spec != null) {
				workloadAppDetails.setAppName(
						(spec.get("externalName").isJsonNull() ? "" : spec.get("externalName").getAsString()));
				workloadAppDetails
						.setAppID((spec.get("externalId").isJsonNull() ? "" : spec.get("externalId").getAsString()));
				workloadAppDetails
						.setAppTag((spec.get("defaultTag").isJsonNull() ? "" : spec.get("defaultTag").getAsString()));
	...
	private JsonObject getApiResponse(String api) {
		...
		if (StringUtils.isEmpty(workloadEndpointToken)) {
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.ACTION, "getApiResponse").put(LogMessage.MESSAGE, "Invalid workload token")
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					.put(LogMessage.USER, ThreadLocalContext.getCurrentMap().get(LogMessage.USER))
					.put(LogMessage.ACTION, "getApiResponse").put(LogMessage.MESSAGE, "Faile to create httpClient")
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL)).build()));
		}
	...
	public ResponseEntity<String> getWorkloadDetailsByAppName(String appName) {
		...
		try {
			...
			resp = adminservice.getTMOAppMetadataDetails();
			Gson gson = new Gson();
			JsonArray results = gson.fromJson(resp, JsonArray.class);

			for (JsonElement jsonElement : results) {
				spec = jsonElement.getAsJsonObject();

				if (spec.get("externalName").toString().equals("\"" + appName + "\"")) {
					response = appName;
		...
		if (null == response) {
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body("{\"errors\":[\"Application name doesn't " + "exist\"]}");
		} else {
			return ResponseEntity.status(HttpStatus.OK).body(spec.toString());
		}
	}
	 ...
	 * @return
	 */
	public List<TMOMetaData> getAllApplicationDetailsFromCLM() {
		List<TMOMetaData> tmoAppMetadataList = new ArrayList<>();
		...
		if (response != null) {
			...
			for (JsonElement jsonElement : results) {
				if (jsonElement.getAsJsonObject() != null) {
					JsonObject spec = jsonElement.getAsJsonObject();
					TMOMetaData tmoAppMetadata = new TMOMetaData();
					if (spec != null) {
						if (spec.get("aliasTag") != null) {
							tmoAppMetadata.setApplicationName(
									(spec.get("aliasTag").isJsonNull() ? "" : spec.get("aliasTag").getAsString()));
						}
						if (spec.get("applicationTag") != null) {
							tmoAppMetadata.setApplicationTag((spec.get("applicationTag").isJsonNull() ? ""
									: spec.get("applicationTag").getAsString()));
						}
						/*
						 * if (spec.get("brtContactEmail") != null) {
						 * tmoAppMetadata.setApplicationOwnerEmailId((spec.get("brtContactEmail").
						 * isJsonNull() ? "" : spec.get("brtContactEmail").getAsString())); } if
						 * (spec.get("projectLeadEmail") != null) {
						 * tmoAppMetadata.setProjectLeadEmailId((spec.get("projectLeadEmail").isJsonNull
						 * () ? "" : spec.get("projectLeadEmail").getAsString())); }
						 */
						if (spec.get("createdBy") != null) {
							tmoAppMetadata.setApplicationOwnerEmailId(
									(spec.get("createdBy").isJsonNull() ? "" : spec.get("createdBy").getAsString()));
						}
						if (spec.get("createdBy") != null) {
							tmoAppMetadata.setProjectLeadEmailId(
									(spec.get("createdBy").isJsonNull() ? "" : spec.get("createdBy").getAsString()));
						}
	 ...
	 * @return
	 */
	public List<TMOMetaData> getAllAppMetadata(String token) {
	 ...
	 * @return
	 */
	private List<TMOMetaData> getAllAppDetails(String token, JsonArray jsonArray) {
		List<TMOMetaData> tmoAppMetadataList = new ArrayList<>();
		ObjectMapper objectMapper = new ObjectMapper();
		if (!ObjectUtils.isEmpty(jsonArray)) {
			for (int i = 0; i < jsonArray.size(); i++) {
				String pathStr = TVaultConstants.TMO_APP_METADATA_PATH + "/" + jsonArray.get(i).getAsString();
				Response readResponse = reqProcessor.process("/tmo-applicaions", "{\"path\":\"" + pathStr + "\"}",
						token);
				if (HttpStatus.OK.equals(readResponse.getHttpstatus())) {
					try {
						TMOAppMetadata tmoAppMetadata = objectMapper.readValue(readResponse.getResponse(),
								new TypeReference<TMOAppMetadata>() {
								});
						log.info(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
								.put(LogMessage.USER, ThreadLocalContext.getCurrentMap().get(LogMessage.USER))
								.put(LogMessage.ACTION, "getAllAppDetails")
								.put(LogMessage.MESSAGE,
										String.format("Parsed metadata for application [%s]",
						...
						log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
								...
								.put(LogMessage.ACTION, "getAllAppDetails")
								.put(LogMessage.MESSAGE,
										String.format("Failed to parse metadata for application [%s]",
												jsonArray.get(i).getAsString()))
								.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL))
								.build()));
					}
					...
					log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
							...
							.put(LogMessage.ACTION, "getAllAppDetails")
							.put(LogMessage.MESSAGE,
									String.format("Metadata not found for application [%s]",
											jsonArray.get(i).getAsString()))
							.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL))
							.build()));
				}
	 ...
	 * @return
	 */
	public Response udpateApplicationMetadata(String token, TMOMetaData currentApp, TMOMetaData clmApp) {
		ObjectMapper objectMapper = new ObjectMapper();
		...
		try {
			...
			log.debug(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.MESSAGE,
							String.format("Trying to udpate metadata for application [%s]",
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.ACTION, "udpateApplicationMetadata")
					.put(LogMessage.MESSAGE,
							String.format("Failed to stringify metadata for application [%s]",
									currentApp.getApplicationName()))
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL)).build()));
		}
		log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
				...
				.put(LogMessage.ACTION, "udpateApplicationMetadata")
				.put(LogMessage.MESSAGE,
						String.format("Failed to udpate metadata for application [%s]",
								currentApp.getApplicationName()))
				.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL)).build()));
